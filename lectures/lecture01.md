...
In C++, this is done by placing the interface in a `.h` file and the implementation in a `.cpp` file. Source code that requires knowledge of the interface must include the interface file (e.g., `#include Interf.h`). To avoid including files multiple times, a few preprocessor commands are used in the interface file:
...
Note that simply writing `auto whichList = theLists[ myhash( x, theLists.size( ) ) ];` wouldn't work because this would create a copy, and 
